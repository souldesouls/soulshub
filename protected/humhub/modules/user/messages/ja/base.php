<?php

return [
    '<strong>Following</strong> user' => '<strong>フォロー</strong> ユーザー',
    '<strong>Member</strong> in these spaces' => '<strong>スペース</strong> 一覧',
    '<strong>User</strong> followers' => '<strong>ユーザー</strong> フォロワー',
    '<strong>User</strong> tags' => '<strong>ユーザー</strong> タグ',
    'A baseline measurement of your mental influence will be taken. Focus on what a wonderful human being you are as you press that button!' => 'あなたの心・想いを測りますので、登録ボタンを押す際は自分がどれだけ素晴らしい人かということに集中してみてくださいね！',
    'Allows access to your about page with personal information' => '他のユーザーが、あなたのプロフィールページを閲覧できるようにする',
    'Basic Settings' => '基本設定',
    'Change Email' => 'メールの変更',
    'Change Password' => 'パスワードを変更する',
    'Change Username' => 'ユーザー名の変更',
    'Confirm new password' => '新しいパスワードを再入力',
    'Connect account' => 'アカウントを接続する',
    'Connected Accounts' => '接続されたアカウント',
    'Connected accounts' => '接続されたアカウント',
    'Created at' => 'で作成',
    'Created by' => 'によって作成された',
    'Current password' => '現在のパスワード',
    'Currently in use' => '現在使用中',
    'Default Group' => 'デフォルトのグループ',
    'Default Space' => 'デフォルトのスペース',
    'Delete' => '削除',
    'Delete Account' => 'アカウントを削除する',
    'Description' => '説明',
    'Disconnect account' => 'アカウントを切断する',
    'E-Mail is already in use! - Try forgot password.' => 'このメールアドレスはすでに使われています。',
    'Email' => 'メール',
    'Follow' => 'フォロー',
    'Force password change upon first login' => '初回ログイン時にパスワードの強制変更',
    'General' => '一般設定',
    'Language' => '言語',
    'Last Login' => '最終ログイン',
    'Modules' => 'モジュール',
    'My Account' => 'アカウント',
    'Name' => '名前',
    'New password' => '新しいパスワード',
    'No users found.' => 'ユーザーはいません',
    'Notifications' => '通知',
    'Password' => 'パスワード',
    'Permission' => '権限',
    'Profile' => 'プロフィール',
    'Register now and participate!' => '登録してください',
    'Security' => 'セキュリティ',
    'Settings' => '設定',
    'Sign in' => 'サインイン',
    'Sign in / up' => 'サインイン/登録',
    'Sign up' => '登録',
    'Sign up now' => '今すぐ登録',
    'Sort order' => '並び順',
    'Status' => 'ステータス',
    'Tags' => 'タグ',
    'Unfollow' => 'フォローを外す',
    'Updated at' => 'で更新',
    'Updated by' => '更新されました。',
    'User' => 'ユーザー',
    'User has been invited.' => 'ユーザーが招待されました',
    'Username' => 'ユーザー名',
    'View your about page' => 'あなたのページの閲覧許可',
    'Visibility' => '可視性',
    'Welcome to %appName%' => 'ようこそ %appName% へ',
    'You got an invite' => '招待を受けました',
    'Your account is disabled!' => 'あなたのアカウントは無効です',
    'or' => 'または',
    '%userId%, welcome to %appName%!' => '%userId%、%appName%へようこそ！',
    '%userId%, welcome to %appName%! Your account has been created. Please click on the button below to login.' => '%userId%、%appName%へようこそ！アカウントが登録されましたので、以下をクリックしてログインしてください。',
    'Account created for %userId% at %appName%' => '%appName%の「%userId%」アカウントが登録されました。',
    'Auth Mode' => '',
    'Enable Notifications' => '',
    'Here you can connect to external service provider for using external services like a single sign on authentication.' => '',
    'Login' => '',
    'Manager' => '',
    'Missing E-Mail Attribute from AuthClient.' => '',
    'Missing ID AuthClient Attribute from AuthClient.' => '',
    'Password changed' => '',
    'Password needs to be at least {chars} characters long.' => '',
    'Profile posts only' => '',
    'Show At Directory' => '',
    'Show At Registration' => '',
    'Show all content' => '',
    'Source' => '',
    'Unknown user status!' => '',
    'Unsuccessful login attempts since last login: {failedLoginAttemptsCount}' => '',
    'Username contains invalid characters.' => '',
    'Welcome to %appName%. Please click on the button below to proceed with your registration.' => '%appName%へようこそ。以下をクリックして登録手続きを進めてください。',
    'You\'re not registered.' => '',
    'You\'ve been invited to join {space} on {appName}' => '',
    'Your account is not approved yet!' => '',
    'Your are already logged in! - Logout first!' => '',
    'Your new password must not be equal your current password!' => '',
    'invited you to join {name}.' => '',
    'invited you to join {space} on {name}.' => '',
];